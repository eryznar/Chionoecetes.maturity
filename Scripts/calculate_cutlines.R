...
# LOG TRANSFORM and BIN DATA ----

chela_db %>% # loaded in get_survey_data.R
  mutate(LN_CW = log(SIZE),
         LN_CH = log(CHELA_HEIGHT)) %>% # transform using natural log
...
min.dat %>%
  ...
  write.csv(paste0(data_dir, "chela_cutline_minima.csv"), row.names = FALSE) # save to Y::drive

min.dat %>%
  dplyr::select(SPECIES, BIN, LOWER, UPPER, MIDPOINT, MINIMUM) %>%
  write.csv("./Output/chela_cutline_minima.csv", row.names = FALSE) # save to repo
